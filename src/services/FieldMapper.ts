...
export class FieldMapper {
	...
	mapFrontmatterToTaskInfo(frontmatter: any, path: string): TaskInfo | null {
		...
		const statusValue =
			frontmatter[propNames.status] ?? frontmatter.complete;
		...
		// People: use configured property name
		const people = Array.isArray(frontmatter[propNames.people])
			? frontmatter[propNames.people].filter(
					(p: any) => typeof p === "string" && this.isWikilink(p),
				)
			: [];
		...
		const tags = Array.isArray(frontmatter[propNames.tags])
			? frontmatter[propNames.tags].filter(
	...
	validateTaskFrontmatter(frontmatter: any): boolean {
		...
		if (
			frontmatter[propNames.due] !== undefined &&
			frontmatter[propNames.due] !== null
		) {
			if (typeof frontmatter[propNames.due] === "string") {
				if (!this.isValidDateString(frontmatter[propNames.due])) {
					return false;
		...
		if (
			frontmatter[propNames.completed] !== undefined &&
		...
		// People validation: use configured property name
		if (
			frontmatter[propNames.people] !== undefined &&
			!Array.isArray(frontmatter[propNames.people])
		) {
			return false;
		}
	...
	createDefaultFrontmatter(partial: Partial<TaskInfo>): Record<string, any> {
		...
		return {
			...
			[propNames.people]: partial.people || [],
			[propNames.tags]: tags,
