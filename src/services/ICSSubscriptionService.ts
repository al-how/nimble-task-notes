	...
	async fetchSubscription(): Promise<void> {
		...
		try {
			...
			this.cache = {
				...
				expires: new Date(
					Date.now() + this.CACHE_EXPIRATION,
				).toISOString(),
			};
			console.log(
				`ICSSubscriptionService: Cached ${events.length} events`,
			);
			...
			console.error(
				"ICSSubscriptionService: Failed to fetch calendar:",
				errorMessage,
			);
			...
			if (errorMessage.includes("404")) {
				new Notice(
					"Calendar not found. Check your calendar URL in settings.",
				);
			} else if (
				...
				new Notice(
					"Failed to fetch calendar. Check your internet connection.",
				);
			} else {
				...
				try {
					...
					const icsEvent: ICSEvent = {
						...
						start: this.icalTimeToISOString(start),
							...
							? this.icalTimeToISOString(event.endDate)
							: undefined,
						allDay: start.isDate,
					...
					if (event.isRecurring()) {
						...
						while (
							...
							const instanceStart =
								this.icalTimeToISOString(occurrence);
							...
							if (event.endDate && start) {
								const duration =
									...
									start.toUnixTime();
								const instanceEndTime =
									occurrence.toUnixTime() + duration;
								instanceEnd = new Date(
									instanceEndTime * 1000,
								).toISOString();
							}
