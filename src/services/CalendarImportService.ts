	...
	async importTodaysMeetings(activeNote: TFile): Promise<number> {
		try {
			...
			if (headingIndex === -1) {
				...
				new Notice(
					"No Agenda heading found. Add #### 📆 Agenda to your note.",
				);
				return 0;
			}

			// Extract date from filename (e.g., "2025-11-06.md" -> "2025-11-06")
			const dateFromFilename = this.extractDateFromFilename(
				activeNote.basename,
			);
			const filterDate = dateFromFilename || this.getToday();
			console.log(
				`CalendarImportService: Filtering events for date: ${filterDate}`,
			);

			const events = this.icsService.getAllEvents();
			console.log(
				`CalendarImportService: Got ${events.length} total events from cache`,
			);
			const matchingEvents = this.filterEventsByDate(events, filterDate);
			console.log(
				`CalendarImportService: Filtered to ${matchingEvents.length} events for ${filterDate}`,
			);
			...
			const existingLinks = this.extractWikilinksUnderHeading(
				content,
				headingIndex,
			);
			...
			const newContent = this.insertWikilinksAtHeading(
				content,
				headingIndex,
				newWikilinks,
			);
			...
			new Notice(
				`Imported ${newWikilinks.length} meeting(s) for ${filterDate}`,
			);
