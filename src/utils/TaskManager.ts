...
export class TaskManager extends Events {
	...
	getTaskInfo(path: string): TaskInfo | null {
		...
		// Parse people (array of wikilinks) - use configured property name
	 ...
	 *
	 * @param personWikilink - Person wikilink (e.g., [[John Smith]])
	 * @returns Array of TaskInfo objects
	 */
	getTasksForPerson(personWikilink: string): TaskInfo[] {
		return this.getAllTasks().filter((task) =>
			task.people.includes(personWikilink),
		);
	}
