...
export class LightweightTasksSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Name of the property that stores task completion status (boolean)",
			)
			.addText((text) =>
				text
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Name of the property that stores task due date (YYYY-MM-DD)",
			)
			.addText((text) =>
				text
					.setPlaceholder("due")
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Name of the property that stores task completion date (YYYY-MM-DD, auto-set when task marked complete)",
			)
			.addText((text) =>
				text
					...
					.onChange(async (value) => {
						const trimmed = value.trim();
						if (trimmed) {
							this.plugin.settings.propertyNames.completed =
								trimmed;
							await this.plugin.saveSettings();
						}
					}),
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Name of the property that stores project wikilinks (array)",
			)
			.addText((text) =>
				text
					...
					.onChange(async (value) => {
						const trimmed = value.trim();
						if (trimmed) {
							this.plugin.settings.propertyNames.projects =
								trimmed;
							await this.plugin.saveSettings();
						}
					}),
		...
		new Setting(containerEl)
			.setName("People property name")
			.setDesc(
				"Name of the property that stores people wikilinks (array)",
			)
			.addText((text) =>
				text
					.setPlaceholder("people")
					.setValue(this.plugin.settings.propertyNames.people)
					.onChange(async (value) => {
						const trimmed = value.trim();
						if (trimmed) {
							this.plugin.settings.propertyNames.people = trimmed;
							await this.plugin.saveSettings();
						}
					}),
			);
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Name of the property that stores tags (array, always includes 'task')",
			)
			.addText((text) =>
				text
					.setPlaceholder("tags")
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Name of the property that stores free-text status notes (string)",
			)
			.addText((text) =>
				text
					.setPlaceholder("statusDescription")
					.setValue(
						this.plugin.settings.propertyNames.statusDescription,
					)
					.onChange(async (value) => {
						const trimmed = value.trim();
						if (trimmed) {
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Enable parsing of dates like 'tomorrow', 'next Friday', etc.",
			)
			.addToggle((toggle) =>
				toggle
			...
			.addText((text) =>
				text
					...
					.onChange(async (value) => {
						...
						if (
							!this.plugin.settings.defaultTags.includes("task")
						) {
		...
		new Setting(containerEl)
			.setName("Projects status property")
		...
		new Setting(containerEl)
			.setName("Excluded project statuses")
			.setDesc(
				"Comma-separated status values to exclude from suggestions",
			)
			.addText((text) => {
				text.setPlaceholder("e.g., deprioritized, completed")
					.setValue(
						this.plugin.settings.projectsExcludedStatuses.join(
							", ",
						),
					)
					.onChange(async (value) => {
						this.plugin.settings.projectsExcludedStatuses = value
							.split(",")
							.map((s) => s.trim())
							.filter((s) => s.length > 0);
						await this.plugin.saveSettings();
		...
		new Setting(containerEl)
			.setName("People tag")
			.setDesc("Tag that identifies people files (without #)")
			.addText((text) => {
				text.setPlaceholder("e.g., person")
					.setValue(this.plugin.settings.peopleRequiredTag)
					.onChange(async (value) => {
						this.plugin.settings.peopleRequiredTag = value;
						await this.plugin.saveSettings();
					});
			});

		new Setting(containerEl)
			.setName("People status property")
			.setDesc("Frontmatter property containing status value")
			.addText((text) => {
				text.setPlaceholder("e.g., status")
					.setValue(this.plugin.settings.peopleStatusProperty)
					.onChange(async (value) => {
						this.plugin.settings.peopleStatusProperty = value;
						await this.plugin.saveSettings();
					});
			});

		new Setting(containerEl)
			.setName("Excluded people statuses")
			.setDesc(
				"Comma-separated status values to exclude from suggestions",
			)
			.addText((text) => {
				text.setPlaceholder("e.g., inactive, archived")
					.setValue(
						this.plugin.settings.peopleExcludedStatuses.join(", "),
					)
					.onChange(async (value) => {
						this.plugin.settings.peopleExcludedStatuses = value
							.split(",")
							.map((s) => s.trim())
							.filter((s) => s.length > 0);
						await this.plugin.saveSettings();
